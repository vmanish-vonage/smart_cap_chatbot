...
def allocate_customer_capacity(customer_api_key, request: dict):
    ...
    def compute_allocatable_tps(row):
        try:
            allowed = int(row['profile_allowed_tps'])
            actual = int(row.get('profile_actual_tps', 0))
            return max(0, allowed - actual)
    ...
    def peak_time_overlaps(row):
        carrier_start = int(row['peak_start_time'])
        carrier_end = int(row['peak_end_time'])
